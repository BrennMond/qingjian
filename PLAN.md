...
1. **P3.5 词库管线**：干净来源的词表 + OpenCC 数据装载。
   `schemes/stele-default` 现在只有 30 条演示词，而**其余验收全卡在它上面**
   （没有真词库，对照实验只能比结构、比不了排序）。
   **雾凇那 44 MB 词表不进仓库**——最大几块来源不明或明确限制，
...
| 阶段 | 方案 | 零件数 | P50 | P99 | 常驻内存 | 测试数 |
| --- | --- | --- | --- | --- | --- | --- |
| P1（首次实测） | 拼音（默认装配） | 少 | **241 ns** | 351 ns | 3 MiB | 99 |
| P3 加完零件后 | 拼音（RIME 形状清单） | 多 | **1.55 µs** | — | 4 MiB | — |
| P3 删掉 `Query::composition` 后 | 拼音 | 多 | **531 ns** | 852 ns | 4 MiB | 282 |
| P3 收尾 | 拼音 / 字形码 | 多 / 少 | **542 / 180 ns** | 851 ns | 4 MiB | 322 |
| **阶段 A 之后（当前）** | 拼音 | 多 | **601 ns** | **1.56 µs** | **4 MiB** | **334** |
