...
//! 的坑——这一条就是那个坑的守门人。
//!
//! # 为什么用**按对比集生成**的临时方案，而不是 `schemes/qingjian-default`
//!
...
/// 装配一个**挂了记忆与预测**的引擎。
fn engine_with(dir: &Path, memory: &Arc<FileMemory>) -> qingjian_engine::EngineImpl {
...
fn predictions_are_absent_when_no_prediction_service_is_installed() {
    ...
    let defs = qingjian_schemes::load_dir(dir.path()).expect("装载测试方案");
    let store = Arc::clone(&memory) as Arc<dyn MemoryStore>;
    let services = Services::new(clock()).with_ranker(Arc::new(MemoryRanker::new(store)));
    let engine = qingjian_engine::EngineImpl::with_services(&defs, services).expect("编译");
    let mut s = engine.create_session();
    commit_code(&mut s, &memory, "jia");
    assert!(
