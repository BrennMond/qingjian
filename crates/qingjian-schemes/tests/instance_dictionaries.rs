...
impl Dir {
    ...
    fn candidates(&self, keys: &str) -> Vec<String> {
        let defs =
            qingjian_schemes::load_dir(&self.0).unwrap_or_else(|e| panic!("方案必须能装载：{e}"));
    ...
    fn degradations(&self) -> String {
        let defs = qingjian_schemes::load_dir(&self.0).expect("装载");
...
";
    ...
    let defs = qingjian_schemes::load_dir(&dir.0);
    ...
    match defs {
        ...
        Ok(d) => {
            ...
            let got = qingjian_schemes::load_dir_reporting(&dir.0).expect("至少主词库能装上");
