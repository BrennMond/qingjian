...
use qingjian_core::{Engine, Key, Outcome, Session};
...
fn punctuation_during_composition_is_returned_to_the_system() {
    ...
    assert!(
        ...
        "标点必须**交还给系统**（这是 Qingjian 的有意选择，见本文件文档），实得 {outcome:?}"
    );
    assert_eq!(
        s.composition().input,
        "ni",
