...
impl Probe for qingjian_table::TableLexicon {
    fn supports_prefix(&self) -> bool {
        Lexicon::supports_prefix(self)
    }
    fn lookup(&self, c: &[CodeUnitId]) -> Vec<(String, i32)> {
...
fn the_two_lexicon_implementations_agree_on_every_capability() {
    let p = std::env::temp_dir().join(format!("qingjian-lexcap-{}.table", std::process::id()));
    let _ = std::fs::remove_file(&p);
    let fp = qingjian_table::BuildFingerprint::of(
    ...
    let table = qingjian_table::TableLexicon::open_checked(&p, Some(fp)).expect("部署词库");
