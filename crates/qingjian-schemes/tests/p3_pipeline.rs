...
fn engine() -> EngineImpl {
    let defs = qingjian_schemes::load_dir(&dir()).unwrap_or_else(|e| {
...
fn the_rime_style_engine_list_is_assembled_in_order() {
    let defs = qingjian_schemes::load_dir(&dir()).expect("装载");
    let def = &defs[0];
...
#[allow(clippy::borrowed_box)]
fn type_into(
    p: &mut Box<dyn qingjian_core::Pipeline + Send>,
    state: &mut qingjian_core::SessionState,
    text: &str,
) {
    for c in text.chars() {
...
fn punctuation_is_mapped_and_committed() {
    ...
    assert!(
        qingjian_core::is_exact(s.candidates()[0].origin, s.candidates()[0].attr),
        "标点是原样上屏，不是猜的"
    );
...
fn a_lexicon_without_prefix_support_simply_does_not_complete() {
    ...
    use qingjian_core::{CandidateSink, CodeUnitId, Lexicon, Score};
    use qingjian_engine::lexicon::InMemoryLexicon;

    let a = qingjian_core::CodeAlphabet::new(vec!["ni".into(), "hao".into()]);
...
fn ascii_composer_switches_and_then_gives_keys_back_to_the_system() {
    ...
    assert!(
        events.iter().any(|e| matches!(
            e,
            qingjian_core::Event::OptionChanged { name, on: true } if name == "ascii_mode"
        )),
...
fn a_user_patch_is_merged_and_its_provenance_is_recorded() {
    ...
    let patch = "menu:\n  page_size: 7\nschema:\n  name: 我改过的名字\n";
    let loaded = qingjian_schemes::load_scheme_layered(
        &base,
        "p3features.schema.yaml",
        Some((patch, "p3features.custom.yaml")),
        &qingjian_dict::DirSource::new(dir()),
    )
    .expect("补丁必须能合并");
...
fn the_directory_loader_also_applies_the_user_patch() {
    ...
    // 而单元测试很容易只覆盖前者。
    let tmp = std::env::temp_dir().join(format!("qingjian-patch-test-{}", std::process::id()));
    ...
    let defs = qingjian_schemes::load_dir(&tmp).expect("目录装载");
    assert_eq!(
    ...
    let layered = qingjian_schemes::load_dir_layered(&tmp).expect("分层目录装载");
