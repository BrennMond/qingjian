...
//! **表 1**（同词表对照）：
//!
//! | 输入 | librime | Qingjian（修复前） | Qingjian（现在） |
//! | --- | --- | --- | --- |
//! | `nihao` | 你好 | 你好 | 你好 |
...
use qingjian_core::{Candidate, Engine, Key, Origin, Span, SpellingAttr};
...
impl Fixture {
    ...
    fn candidates(&self, input: &str) -> Vec<Candidate> {
        let defs = qingjian_schemes::load_dir(&self.root)
...
fn committing_a_prefix_candidate_keeps_the_remainder_in_the_input() {
    ...
    let defs = qingjian_schemes::load_dir(&f.root).expect("夹具");
    ...
    let outcome = s.select(idx, qingjian_core::SelectionSource::Keyboard);
    assert!(
        matches!(outcome, qingjian_core::Outcome::Committed(_)),
        "选中前缀候选应当上屏：{outcome:?}"
    );
    assert_eq!(
        s.composition().input,
...
fn committing_a_whole_input_candidate_clears_the_input() {
    ...
    let idx = s
        ...
        .position(|c| c.text == "你好")
        .expect("应当有「你好」");
    let _ = s.select(idx, qingjian_core::SelectionSource::Keyboard);
    assert!(
