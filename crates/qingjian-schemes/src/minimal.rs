...
//!
//! 1. **单一数据来源**：方案就在 `schemes/qingjian-default/*.yaml`，
//!    不再有"Rust 里一份、文件里一份"的漂移。
//! 2. **真实路径被测试**：`qingjian` 一启动走的就是**真正的解析器**，
//!    而不是一条"只有测试才会走"的旁路。**旁路从来不坏，也从来不证明什么。**
//! 3. **换方案不用重编译**：用户可以把自己的方案放进目录，用 `--scheme-dir` 指过来。
//!
//! # 为什么还要内嵌
//!
//! 因为 `qingjian` 是个**命令行工具**，可能从任何目录被调用。
...
use qingjian_core::SchemaError;
use qingjian_dict as dict;
use qingjian_engine::scheme::{SchemeDef, TranslatorKind};
...
pub const EMBEDDED: &[(&str, &str)] = &[
    ...
    // `pinyin.schema.yaml` 前面，`qingjian --scheme-dir schemes/qingjian-default`
    // 默认就会用它——于是 41 万条的真实词库永远用不上
    // （实测症状：`weixiao` 出字面量）。
    (
        "z-pinyin-demo.schema.yaml",
        include_str!("../../../schemes/qingjian-default/z-pinyin-demo.schema.yaml"),
    ),
    (
        "cn_dicts/base.dict.yaml",
        include_str!("../../../schemes/qingjian-default/cn_dicts/base.dict.yaml"),
    ),
    (
        "shape.schema.yaml",
        include_str!("../../../schemes/qingjian-default/shape.schema.yaml"),
    ),
    (
        "shape.dict.yaml",
        include_str!("../../../schemes/qingjian-default/shape.dict.yaml"),
    ),
];
...
mod tests {
    ...
    fn embedded_demo_dictionary_is_loaded() {
        ...
        let p = defs
            ...
            .find(|d| d.info.schema_id == "pinyin-demo")
            .unwrap();
        let qingjian_engine::scheme::DictSource::Inline(entries) = &p.dictionary else {
            panic!("内嵌方案应当用内联词条");
        };
        assert!(
            entries.len() > 10,
        ...
        // 的部署路径（见 `tools/README.md` 与 `qingjian-cli` 的自动发现）。
    ...
    fn family_is_declared_for_shared_user_dictionary() {
        ...
        let p = defs
            ...
            .find(|d| d.info.schema_id == "pinyin-demo")
            .unwrap();
        assert_eq!(p.info.family.as_deref(), Some("qingjian-pinyin"));
    }
    ...
    fn scheme_data_is_simplified_only() {
        ...
        for d in all().unwrap() {
            let qingjian_engine::scheme::DictSource::Inline(entries) = &d.dictionary else {
                continue;
            };
