...
//! 架构位置：`qingjian-schemes` 的装载入口；`qingjian-config` 的 `$ref` / 补丁机制
//! 在这里被**真正接进装载路径**。
//!
...
use qingjian_config::{MergeMode, Node, Value};
...
impl Resolution {
    ...
    pub fn of(layers: Vec<(Layer, Node)>) -> Result<Self, String> {
        ...
        for (idx, (layer, node)) in layers.into_iter().enumerate() {
            ...
            qingjian_config::merge_into(&mut root, &node, MergeMode::Layer)
...
mod tests {
    ...
    fn later_layers_win_and_origins_say_so() {
        ...
        // 合并结果取后一层。
        let size = qingjian_config::lookup(&r.root, "menu/page_size").unwrap();
    ...
    fn a_layer_may_replace_a_scalar_with_a_map() {
        ...
        .unwrap();
        assert_eq!(
            qingjian_config::lookup(&r.root, "menu/page_size")
                .and_then(qingjian_config::Node::as_int),
            Some(9),
            "后一层的映射整体替换了前一层的标量"
        );
