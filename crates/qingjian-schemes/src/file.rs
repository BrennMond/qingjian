...
//! can use directly.
//! 架构位置：`qingjian-config` / `qingjian-dict` 与 `qingjian-engine` 之间的桥。
//!
...
use qingjian_config::{Node, Value};
use qingjian_core::Score;
use qingjian_core::{CodeAlphabet, CodeUnitId, Diagnostic, SchemaError, SchemaInfo, Switch};
use qingjian_dict as dict;
use qingjian_engine::keyspec::parse_key_name;
use qingjian_engine::pipeline::CANDIDATE_CAP;
use qingjian_engine::scheme::{entry, SchemeDef, TranslatorKind, SCHEME_FORMAT_VERSION};
...
/// 它只拿 [`SchemeDef`]。这条分界线让 `qingjian-engine` 保持零配置概念。
#[derive(Debug)]
pub struct Loaded {
    /// 编译前的方案声明。
    pub def: SchemeDef,
...
fn load_layered_with(
    ...
    // **编译的是合并结果**，不是原始方案文件。
    //
    // 这一行是 P2 欠下的接线：`qingjian-config` 里的分层补丁早就实现并测过，
...
fn find_patch(dir: &std::path::Path, text: &str, file_name: &str) -> Option<(String, String)> {
    let id = qingjian_config::parse(text)
        .ok()
        .and_then(|root| {
            root.get("schema")
...
fn parse_or_diag(text: &str, path: &str) -> Result<Node, SchemaError> {
    qingjian_config::parse(text).map_err(|e| SchemaError::Invalid {
        schema_id: path.to_owned(),
...
fn load_scheme_with(
    ...
    let root = qingjian_config::parse(text).map_err(|e| SchemaError::Invalid {
        schema_id: path.to_owned(),
        diagnostics: vec![Diagnostic::new(
            path,
            format!("第 {} 行：{}", e.line, e.message),
...
fn load_from_root(
    ...
    let mut external: Option<std::sync::Arc<dyn qingjian_core::Lexicon>> = None;
    match dict_name {
    ...
    // 短写法缺省时可以从列表里推断翻译器族。
    let mut coverage: Option<qingjian_engine::registry::CoverageReport> = None;
    ...
    if let Some(eng) = root.get("engine") {
        ...
        if !names.is_empty() {
            let rep = qingjian_engine::registry::CoverageReport::of(&names);
            // 供 `--dump-config` 打印。
    ...
    let mut tags = qingjian_engine::tag::TagTable::new();
    let mut engine_spec = qingjian_engine::spec::EngineSpec::default();
    if let Some(eng) = root.get("engine") {
    ...
    // 这份默认来自**引擎自带的预设**（见 `qingjian_engine::presets`），
    // 而不是某个具体输入法的数据。
    let punctuator = if let Some(n) = root.get("punctuator") {
        components::read_punctuator(n, &mut diags, path)
    } else {
        let p = qingjian_engine::presets::qingjian();
        qingjian_engine::spec::PunctuatorSpec {
    ...
    let navigator = root.get("navigator").map_or_else(
        || {
            ...
            let p = qingjian_engine::presets::qingjian();
            qingjian_engine::spec::NavigatorSpec {
    ...
    let mut inline = qingjian_engine::scheme::InlineConfigs::default();
    ...
    let mut affixes: Vec<(String, qingjian_engine::spec::AffixSpec)> = Vec::new();
    let mut reverse_lookups: Vec<(String, qingjian_engine::spec::ReverseLookupSpec)> = Vec::new();
    let mut converters: Vec<(
        String,
        std::collections::BTreeMap<String, Vec<String>>,
        qingjian_engine::spec::SimplifierSpec,
    )> = Vec::new();
    // 每个 `simplifier@别名` 的**外部数据事实**：它的转换表到底装到了没有、
    // 从哪个文件装的。由装载器填（只有它知道文件在不在），最后交给
    // `qingjian_engine::registry::unmet_requirements` 决定要不要出声。
    ...
    {
        let (component, alias) = qingjian_engine::spec::split_alias(name);
        ...
        match component {
            ...
            "simplifier" => {
                ...
                if let Some(t) = block.get("table").and_then(qingjian_config::Node::as_map) {
                    for (k, v) in t {
    ...
    let mut translator_specs: Vec<(String, qingjian_engine::spec::TranslatorSpec)> = Vec::new();
    ...
    let mut extra_lexicons: Vec<(String, qingjian_engine::scheme::DictSource)> = Vec::new();
    for name in &engine_spec.translators {
        ...
        if let Some(block) = root.get(a) {
            ...
            if let Some(dict_name) = spec.dictionary.as_deref().filter(|d| !d.is_empty()) {
                let loaded: Result<qingjian_engine::scheme::DictSource, Diagnostic> = match *mode {
                    DictMode::Inline => {
                        match dict::load_with_imports(dicts, dict_name, dict_name) {
                            Ok(l) => Ok(qingjian_engine::scheme::DictSource::Inline(
                                l.entries
                                    .iter()
                    ...
                    DictMode::Deployed(cache) => {
                        deploy_dict(dicts, dict_name, &alphabet_ids, cache)
                            .map(qingjian_engine::scheme::DictSource::External)
                            .map_err(|d| {
                                Diagnostic::new(
                                    path,
                                    format!(
    ...
    let page_size = root
        ...
        .unwrap_or(qingjian_engine::pipeline::DEFAULT_PAGE_SIZE);
    ...
    let tag: qingjian_core::Tag = Box::leak(tag_text.into_boxed_str());
    ...
    Ok(Loaded {
        def: SchemeDef {
            ...
            dictionary: match external {
                Some(l) => qingjian_engine::scheme::DictSource::External(l),
                None => qingjian_engine::scheme::DictSource::Inline(entries),
            },
...
fn leading_char_of_pattern(
    recognizer: &qingjian_engine::spec::RecognizerSpec,
    name: &str,
) -> Option<char> {
...
fn opencc_manifest(dir: Option<&std::path::Path>) -> Vec<(String, ManifestEntry)> {
    ...
    let Ok(root) = qingjian_config::parse(&text) else {
        return Vec::new();
    };
...
///
/// 真正的解析在 `qingjian-dict`（它是纯函数、可脱离文件系统测试）；
...
/// 把一张转换表并进另一张：**逐键覆盖**。
///
/// 用于"`OpenCC` 的链"（后面的词典赢）——`qingjian-dict` 内部已经按链的
...
fn external_data_facts(
    root: &Node,
    engine: &qingjian_engine::spec::EngineSpec,
    convert_facts: &std::collections::BTreeMap<String, ConvertFact>,
) -> Vec<qingjian_engine::registry::ExternalData<'static>> {
    let mut out = Vec::new();
    for name in engine.translators.iter().chain(engine.filters.iter()) {
        ...
        out.push(qingjian_engine::registry::ExternalData {
            alias: alias_static,
            present,
        });
    }
...
/// 编译走"同目录唯一临时文件 + 原子改名"（见 `qingjian_table::compile`），
...
fn deploy_dict(
    ...
    cache_dir: &std::path::Path,
) -> Result<std::sync::Arc<dyn qingjian_core::Lexicon>, Diagnostic> {
    ...
    let units: Vec<String> = (0..alphabet.len())
        ...
        .map(str::to_owned)
        .collect();
    let fingerprint = qingjian_table::BuildFingerprint::of(
        qingjian_table::FORMAT_VERSION,
        qingjian_table::COMPILER_OPTIONS,
        &units,
        checksum,
    );
    ...
    if !table_path.exists() {
        ...
        let r = qingjian_table::compile(
            ...
            |w| {
                ...
                .map(|_| ())
                .map_err(|e| qingjian_table::CompileError::Io(e.to_string()))
            },
            &table_path,
        );
        if let Err(e) = r {
    ...
    let lex = qingjian_table::TableLexicon::open_checked(&table_path, Some(fingerprint))
...
";
    ...
    fn loads_a_well_formed_scheme() {
        ...
        let qingjian_engine::scheme::DictSource::Inline(e) = &d.dictionary else {
            panic!("应当走内联路径");
        };
        assert_eq!(e.len(), 1);
        assert_eq!(e[0].1, "你好");
    ...
    fn abbrev_accepts_the_shorthand_form() {
        ...
        match &d.rules[0] {
            Rule::Derive { attr, .. } => {
                assert!(attr.contains(qingjian_core::SpellingAttr::ABBREV));
            }
