//! # qingjian-schemes — 方案资产与装载
//!
//! 中文职责：把 `.schema.yaml` + `.dict.yaml` 装载成引擎能用的方案；
//! 并内嵌一份默认方案，使 `qingjian` 在没有配置文件时也能用。
//! English role: load `.schema.yaml` + `.dict.yaml` into engine-ready schemes,
//! and embed a default set so `qingjian` works with no config files present.
//!
//! 架构位置：**方案资产**，与内核分属不同 crate（PLAN D24）。
//! 这一个 crate 的存在本身就是一条架构约束的可执行表达：
//! 内核（`qingjian-core` / `qingjian-engine`）里不允许出现"拼音 / 音节"这类词汇，
...
//!
//! ```
//! use qingjian_core::{Engine, Key, KeyCode, Modifiers, NamedKey, Outcome};
//!
//! let engine = qingjian_engine::EngineImpl::new(&qingjian_schemes::minimal::all().unwrap()).unwrap();
