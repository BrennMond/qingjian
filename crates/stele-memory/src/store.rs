...
//! 架构位置：`stele-memory` 的核心，实现 `stele_core::MemoryStore`。
//!
//! # 三条硬约束（P4a 的验收）
//!
//! 1. **按键时零磁盘 I/O**（红线）。`record` / `lookup` / `forget` 只改内存，
//!    落盘只由显式的 [`FileMemory::flush`] 触发。这条有测试：
//!    `crates/stele-memory/tests/no_disk_io_on_keypath.rs` 读 `/proc/self/io`。
...
/// 实测（`stele-bench --scheme-dir schemes/stele-default --userdb <文件>
...
/// `stele-bench --scheme-dir schemes/stele-default --schema=pinyin
...
/// [`stele_core::Candidate::key`] 随候选带出（PLAN D42）。本函数是
...
fn degraded_clock() -> Arc<dyn Clock> {
    Arc::new(stele_core::FrozenClock {
        secs: 0,
        ms: 0,
        offset_secs: 0,
    })
...
mod tests {
    use super::*;
    use stele_core::{FrozenClock, SpellingAttr};
    ...
    fn record_then_lookup_finds_it() {
        ...
        assert!(got[0].bonus > stele_core::Score::ZERO);
    }
    ...
    fn time_decays_the_bonus_across_reloads() {
        ...
        let dir = std::env::temp_dir().join(format!("stele-mem-decay-{}", std::process::id()));
    ...
    fn predictions_survive_flush_and_reload() {
        let dir = std::env::temp_dir().join(format!("stele-pred-{}", std::process::id()));
        let path = dir.join("user.mem");
        let _ = std::fs::remove_file(&path);
    ...
    fn flush_writes_and_reload_restores() {
        let dir = std::env::temp_dir().join(format!("stele-mem-test-{}", std::process::id()));
        let path = dir.join("user.mem");
        let _ = std::fs::remove_file(&path);
    ...
    fn a_corrupt_file_degrades_instead_of_failing() {
        let dir = std::env::temp_dir().join(format!("stele-mem-corrupt-{}", std::process::id()));
        let path = dir.join("user.mem");
