...
//! 架构位置：`stele-memory` 的消费者一侧；前端（CLI / TSF / Android）
//! 每个按键之后调用一次。
//!
...
//! # use stele_memory::{FileMemory, apply_events};
...
use stele_core::{Commit, Event, MemoryStore, Trigger};
...
mod tests {
    use super::*;
    use std::sync::Mutex;
    use stele_core::{Context, Lane, MemoryEntry, Origin, Prediction, SpellingAttr};
