...
//! 架构位置：`stele-memory` 的消费者一侧；`stele-engine` 在装配流水线时
...
use stele_core::{clamp_bonus, is_exact, Candidate, Lane, MemoryStore, QueryView, Ranker, Score};
...
mod tests {
    ...
    impl MemoryStore for FakeMemory {
        ...
        fn predict_next(&self, _context: &Context) -> Vec<stele_core::Prediction> {
            Vec::new()
        }
    }
}
