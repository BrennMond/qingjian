...
use stele_core::{Clock, Commit, FrozenClock, Lane, MemoryStore, Origin, SpellingAttr, Trigger};
...
impl Db {
    ...
    fn open(&self, cap: usize) -> stele_memory::FileMemory {
        stele_memory::FileMemory::open(&self.path, clock(0), cap).expect("打开记忆")
    }

    fn texts(m: &stele_memory::FileMemory, key: &str) -> Vec<String> {
...
fn a_corrupt_file_is_not_overwritten_by_flushing() {
    ...
    std::fs::write(&db.path, b"this is not a stele memory file").unwrap();

    let (m, note) = stele_memory::FileMemory::open_or_degrade(&db.path, clock(0), 100);
    ...
    assert_eq!(
        raw, b"this is not a stele memory file",
        "坏文件必须原样保留"
    );
}
