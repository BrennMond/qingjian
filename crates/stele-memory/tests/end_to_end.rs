...
//!
//! # 为什么用自建的临时方案，而不是 `schemes/stele-default`
//!
...
/// 装配一个挂了记忆的引擎。
fn engine_with(dir: &std::path::Path, memory: &Arc<FileMemory>) -> stele_engine::EngineImpl {
...
fn demo_engine(memory: &Arc<FileMemory>) -> stele_engine::EngineImpl {
    let defs = stele_schemes::all().expect("内嵌方案必须能装载");
    let services = Services::new(clock()).with_ranker(Arc::new(MemoryRanker::new(
        Arc::clone(memory) as Arc<dyn MemoryStore>,
    )));
    stele_engine::EngineImpl::with_services(&defs, services).expect("编译内嵌方案")
}
