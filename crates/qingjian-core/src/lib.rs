//! # Qingjian IME core / 青简内核抽象层
//!
//! 中文职责：定义引擎的一切抽象——`Engine` / `Session` 两级对象、五类骨架组件的
//! trait、外部服务的 trait，以及贯穿全局的数据结构。
//! English role: the abstraction layer of the Qingjian input-method engine —
...
//!
//! ```
//! use qingjian_core::{is_exact, Origin, Score, SpellingAttr};
//!
...
pub use error::{Diagnostic, QingjianError, Result, SchemaError};
...
pub use service::{
    ...
    FrozenClock, Lexicon, LiteralSpelling, MemoryEntry, MemoryStore, NoMemory, PathLimits,
    PathSink, Prediction, PredictionOrigin, QueryView, RandomSource, Ranker, Services, Spelling,
    SpellingPath, SystemRandom,
};
pub use session::{
