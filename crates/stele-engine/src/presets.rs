...
//! `import_preset: <name>`.
//! 架构位置：`stele-engine` 的数据表；由方案装载器取用。
//!
...
//! 所以：
//!
//! - 我们提供**自己的**预设（名字是 [`PRESET_STELE`]），内容是"一个中文
...
/// 我们提供的预设名。
pub const PRESET_STELE: &str = "stele";
...
mod tests {
    ...
    fn the_named_preset_exists_and_is_not_empty() {
        let p = get(PRESET_STELE).expect("默认预设必须在");
