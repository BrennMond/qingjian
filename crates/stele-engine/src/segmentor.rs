...
/// # 前缀写两个字符是什么意思
///
/// RIME 约定：`prefix: "uU"` 表示**大小写两种写法都接受**（`u` 与 `U`）。
/// 所以它不是一个两字符的前缀，而是**两个候选前缀**。
/// 我们把这条约定落在 [`AffixSpec::prefix`] 的解读里：
/// 长度 2 且两个字符是同一个字母的大小写时，按两个前缀处理；
/// 否则（例如长度 1、或 `zz` 这种）当作**整个字符串就是一个前缀**。
///
/// 这个判断有歧义（`"ab"` 到底是"前缀 ab"还是"a 或 b"？），所以
/// **装载器在长度恰好为 2 且两字符不同源时会报一条诊断**，让人显式写清楚。
