...
use stele_core::{Engine, Key, Session};
...
impl Dir {
    ...
    fn session(&self) -> Box<dyn Session + Send> {
        let defs = stele_schemes::load_dir(&self.0).expect("方案必须能装载");
...
fn initial_quality_actually_changes_the_scores() {
    ...
    // 那正是"逐实例"而不是"全局"的证据。
    let expect = stele_core::Score::from_weight(1.1).as_milli_log();
    let mut checked = 0;
...
";
    ...
    let defs = stele_schemes::load_dir(&d.0).expect("方案");
