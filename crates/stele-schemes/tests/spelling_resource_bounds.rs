...
use stele_core::{CodeAlphabet, Engine, Expansion, ExpansionSink, Key};
use stele_engine::spelling::{ExpansionLimits, ExpansionStats, SpellingTable};

/// 真实默认拼音方案的拼写表（399 个音节 + 一条缩写规则）。
///
/// 用**内嵌的演示孪生体**（`z-pinyin-demo`）：它的 `alphabet` 与 `rules`
/// 与 `schemes/stele-default/pinyin.schema.yaml` 完全一致，只是词库指向
/// 手写的小词典（见 `minimal.rs` 的模块文档）。因此这里测的是
/// **真实的字母表规模**，却不必在测试里编译 41 万词条。
fn real_pinyin_table() -> SpellingTable {
    let defs = stele_schemes::all().expect("内嵌方案必须能装载");
    let p = defs
        .iter()
        .find(|d| d.translator == stele_engine::scheme::TranslatorKind::SpellingGraph)
...
fn normal_words_are_still_recalled() {
    ...
    let engine = stele_engine::EngineImpl::new(&defs).expect("引擎");
