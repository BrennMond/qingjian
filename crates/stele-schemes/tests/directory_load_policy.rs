...
fn a_broken_schema_is_skipped_not_fatal() {
    ...
    assert!(
        stele_schemes::load_dir(&dir.0).is_err(),
        "严格入口遇到坏方案必须报错"
    );

    // ② 新的报告入口：好的装上，坏的进 skipped，**且诊断可读**。
    let got = stele_schemes::load_dir_reporting(&dir.0).expect("至少有一个方案能装上");
    ...
    for c in "ni".chars() {
        s.process_key(stele_core::Key::ch(c));
    }
    assert!(
...
fn a_directory_with_only_broken_schemas_is_an_error() {
    ...
    let e = stele_schemes::load_dir_reporting(&dir.0).expect_err("全坏必须报错");
    let msg = e.to_string();
    assert!(!msg.is_empty());
}
...
fn an_empty_directory_is_an_error() {
    ...
    assert!(stele_schemes::load_dir_reporting(&dir.0).is_err());
}
...
fn a_missing_scheme_file_is_reported_not_silently_dropped() {
    ...
    let got = stele_schemes::load_dir_reporting(&dir.0).expect("好方案仍应装上");
