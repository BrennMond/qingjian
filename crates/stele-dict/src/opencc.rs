...
//! 架构位置：`stele-dict`（数据格式层）；消费者是 `stele-schemes` 的方案装载器，
//! 最终变成 `stele-engine` 的 `Converter` 滤镜。
//!
//! # 为什么要单独做这件事
//!
//! `simplifier` 的机制早就实现了（`stele-engine` 的 `Converter`），
...
pub fn parse_table(text: &str, path: &str) -> Result<(ConvertTable, Vec<String>), OpenCcError> {
    ...
    for (i, raw) in text.lines().enumerate() {
        ...
        // 因此切分推迟到消费者（`stele-engine` 的 `Converter`），
        // 它同时握着键与值，能做出正确判断。
        //
