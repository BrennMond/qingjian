...
//! 架构位置：`qingjian-engine` 的上游；产物是**原始词条**，由方案装载器编译成词库。
//!
//! # 文件形状
//!
//! ```text
...
use qingjian_config::{parse_at, Node};
...
";
    ...
    fn parses_header_and_entries() {
        let d = parse_dict(SIMPLE, "qingjian.dict.yaml").unwrap();
        assert_eq!(d.header.name, "qingjian");
