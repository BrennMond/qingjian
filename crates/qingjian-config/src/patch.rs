...
//! 架构位置：`qingjian-config` 在解析之后、类型化之前的一步。
//!
//! # 为什么需要 `$ref`（G1）
//!
...
/// 一组具名的已解析文档（例如 `default`、`qingjian`）。
...
mod tests {
    ...
    fn ref_copies_the_target_node() {
        let out = reg().expand("qingjian").unwrap();
        let half = out
            .get("punctuator")
        ...
        assert_eq!(
            out.get("name").unwrap().as_str().as_deref(),
            Some("qingjian")
        );
    }
    ...
    fn referenced_node_is_not_mutated() {
        // 复制语义：展开之后原文档必须原封不动。
        let r = reg();
        let _ = r.expand("qingjian").unwrap();
