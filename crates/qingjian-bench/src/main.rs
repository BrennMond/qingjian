...
fn main() {
    ...
    if args.iter().any(|a| a == "-h" || a == "--help") {
        println!(
            "qingjian-bench — 称重台\n\n\
             用法：qingjian-bench [--iterations=N] [--json] [--schema=<id>] [--scheme-dir <目录>]\n\
    ...
    let mut buffer: Vec<qingjian_core::Candidate> = (0..200)
        .map(|i| qingjian_core::Candidate {
            text: format!("候选{i}"),
            comment: None,
            score: qingjian_core::Score::from_milli_log(1000),
            origin: qingjian_core::Origin::SystemWord,
            attr: qingjian_core::SpellingAttr::NORMAL,
            span: qingjian_core::Span::new(0, 1),
    ...
    let sort_report = measure(iterations.min(20_000), || {
        qingjian_core::sort_candidates(&mut buffer);
    });
    ...
    // 否则是内嵌的演示方案。
    //
    // **为什么称重台必须支持目录**：`schemes/qingjian-default` 的真实词库有
    ...
    let mut defs: Vec<qingjian_engine::scheme::SchemeDef> = match &scheme_dir {
        Some(dir) => qingjian_schemes::load_dir_deployed(dir, &dir.join(".qingjian-cache"))
            .expect("按 --scheme-dir 装载方案失败"),
        None => qingjian_schemes::all().expect("内嵌方案必须能装载 —— 失败说明打包坏了"),
    };
    ...
    if count_queries {
        for def in &mut defs {
            if let qingjian_engine::scheme::DictSource::External(inner) = &def.dictionary {
                let l = Arc::new(AtomicU64::new(0));
                let p = Arc::new(AtomicU64::new(0));
                def.dictionary =
                    qingjian_engine::scheme::DictSource::External(Arc::new(CountingLexicon {
    ...
    let predict_cap: usize = args
        ...
        .and_then(|v| v.parse().ok())
        .unwrap_or(qingjian_memory::DEFAULT_PREDICT_CAPACITY);
    let clock: Arc<dyn qingjian_core::Clock> = Arc::new(qingjian_memory::SystemClock::new());
    let memory: Option<Arc<qingjian_memory::FileMemory>> = userdb.as_ref().map(|p| {
        let m = if predict_cap == qingjian_memory::DEFAULT_PREDICT_CAPACITY {
            ...
            qingjian_memory::FileMemory::in_memory_with(
                Arc::clone(&clock),
                qingjian_memory::DEFAULT_CAPACITY,
                predict_cap,
            )
    ...
    let services = {
        let base = Services::new(Arc::clone(&clock))
            .with_random(Arc::new(|| Box::new(qingjian_core::SystemRandom::new())));
        let base = match &memory {
            Some(m) => base.with_ranker(Arc::new(qingjian_memory::MemoryRanker::new(
                Arc::clone(m) as Arc<dyn qingjian_core::MemoryStore>,
            ))),
            None => base,
        };
        ...
        let base = match (predict || seed_predict.is_some(), &memory) {
            (true, Some(m)) => {
                base.with_prediction(Arc::clone(m) as Arc<dyn qingjian_core::MemoryStore>)
            }
            (true, None) => {
                eprintln!(
        ...
        match (embed, &memory) {
            (true, Some(m)) => {
                ...
                if let Some(model) = qingjian_embed::VectorMemory::train(
                    samples,
                    qingjian_embed::VectorConfig::default(),
                ) {
                    embed_info = Some((model.len(), model.bytes()));
                    base.with_ranker(Arc::new(qingjian_embed::EmbedRanker::new(Arc::new(model))))
                } else {
    ...
    let engine =
        qingjian_engine::EngineImpl::with_services(&defs, services).expect("默认方案应当能编译");
        ...
        (Some(m), Some(n)) if n > 0 => {
            ...
            for i in 0..n {
                m.record(&qingjian_core::Commit {
                    ...
                    origin: qingjian_core::Origin::Prediction,
                    attr: qingjian_core::SpellingAttr::NORMAL,
                    lane: qingjian_core::Lane::Predict,
    ...
    if let Some(c) = &memory_cost {
        ...
        println!(
            "  默认上限 {} 条（`qingjian_memory::DEFAULT_CAPACITY`）",
            qingjian_memory::DEFAULT_CAPACITY
        );
    }
    if let Some(c) = &prediction_cost {
        ...
        println!(
            "  默认上限 {} 条（`qingjian_memory::DEFAULT_PREDICT_CAPACITY`）；\
             一次学习写 bigram+trigram 两条",
            qingjian_memory::DEFAULT_PREDICT_CAPACITY
        );
    }
    if let Some((vocab, bytes)) = embed_info {
        ...
        println!(
            "  {} 个词 × {} 维 → 向量表 {} KiB（{:.2} MiB）",
            vocab,
            qingjian_embed::VectorConfig::default().dim,
            bytes / 1024,
            mib
        );
        println!(
            "  加成上限：前 {} 名、每次最多 {} 毫对数（`qingjian_embed::EmbedRanker`）",
