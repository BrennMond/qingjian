...
//! 架构位置：`qingjian-core::Processor` 的实现。
//!
//! # 处理器不直接产出 `Commit`
//!
...
//! [`qingjian_core::PendingCommit`] 的说明）。
//!
//! # 处理器之间也要能"转交"
//!
...
impl qingjian_core::Processor for Speller {
    ...
    fn enabled(&self, options: &qingjian_core::Options) -> bool {
...
impl qingjian_core::Processor for Editor {
    fn name(&self) -> &'static str {
        "editor"
    }
...
impl qingjian_core::Processor for AsciiComposer {
    fn name(&self) -> &'static str {
        "ascii_composer"
    }
...
impl qingjian_core::Processor for Navigator {
    fn name(&self) -> &'static str {
        "navigator"
    }
...
impl qingjian_core::Processor for KeyBinder {
    fn name(&self) -> &'static str {
        "key_binder"
    }
...
/// 最后一段的起始字节位置。
fn last_segment_start(c: &qingjian_core::Composition) -> Option<usize> {
...
impl qingjian_core::Processor for Selector {
    fn name(&self) -> &'static str {
        "selector"
    }
...
mod tests {
    ...
    fn speller_can_be_suppressed_by_a_switch() {
        let mut s = state();
        s.options
            .declare(qingjian_core::Switch::new("ascii_mode", true));
    ...
    fn backspace_removes_a_whole_unit_when_segmented() {
        ...
        for (a, b) in [(0usize, 2usize), (2, 5)] {
            let mut seg = qingjian_core::Segment::new(qingjian_core::Span::new(a, b));
            seg.tags.push("abc");
    ...
    fn ascii_composer_toggles_and_then_rejects_printable_keys() {
        let mut s = state();
        s.options
            .declare(qingjian_core::Switch::new("ascii_mode", false));
