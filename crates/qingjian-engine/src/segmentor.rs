...
//! 架构位置：`qingjian-core::Segmentor` 的实现。
//!
//! # 标签是"切分器 → 翻译器"的绑定层（G3）
//!
...
use qingjian_core::{Query, Segment, SegmentStatus, Segmentor, Span, Tag};
...
impl InputScan {
    /// 转成内核侧的**只读视图**（零拷贝）。
    #[must_use]
    pub fn view(&self) -> qingjian_core::InputScanView<'_> {
        qingjian_core::InputScanView {
            claims: &self.claims,
        }
    }
    ...
    /// 在 `pos` 处开始的认领。
    #[must_use]
    pub fn claim(&self, pos: usize) -> Option<&qingjian_core::Claim> {
...
/// 从一个正则源码里取出**全部匹配的共同必需前缀**。
///
/// # 这是一次被实测打脸的"保守优化"
///
/// 旧版本用字符串扫描近似抽取（`^` 之后到第一个元字符之前）。
/// 它的文档写着「取不准时宁可返回空串」，**但实现做不到这一点**：
/// 它把"某一分支或可选项的首字符"当成了必需前缀，于是产生**假阴性**
/// ——该认出来的输入认不出来。审计实测：
///
...
/// | `^a?b$` | `b` | 匹配 | `"a"` | **未认领** |
///
/// 性能优化只能产生**假阳性**（多做一次完整匹配），绝不能产生假阴性。
///
/// 现在改为从**编译后的语法树**上取（[`crate::regex::Regex::required_prefix`]），
/// 它的正确性可以证明：`Concat` 要求各元素依次匹配，`Char` 只匹配它自己，
/// 因此在开头连续取到的字面字符是每一个匹配都必须消费的前缀。
/// 编译失败时返回空串（空前缀人人都匹配 ⇒ 走完整正则那条慢路）。
///
/// # 为什么这里还要"再编译一次"
///
/// 调用点（`components::read_recognizer`）已经编译过一次正则来做错误诊断。
/// 这里为了拿到语法树而再编译一次，代价发生在**装载期**、每个模式一次，
/// 与按键路径无关。用"再编译一次"换掉"手写近似扫描"，是划算的：
/// 后者已经错过三次。
#[must_use]
pub fn leading_literal(regex: &str) -> String {
    crate::regex::Regex::compile(regex).map_or_else(|_| String::new(), |re| re.required_prefix())
}
...
struct CompiledPattern {
    ...
    /// [`Segment::tags`](qingjian_core::Segment::tags) 里放的完全一致
...
impl Recognizer {
    ...
    pub fn scan(&self, input: &str) -> InputScan {
        ...
        if let Some((end, tag)) = best {
            scan.claims
                .push(qingjian_core::Claim { start: 0, end, tag });
...
impl Segmentor for Matcher {
    ...
    fn rescan(&mut self, scan: &qingjian_core::InputScanView<'_>) {
        self.scan = InputScan {
    ...
    fn proceed(&self, _q: &Query<'_>, seg: &mut qingjian_core::Segmentation) -> bool {
        let Some(claim) = self
            .scan
...
impl Segmentor for AffixSegmentor {
    ...
    fn proceed(&self, q: &Query<'_>, seg: &mut qingjian_core::Segmentation) -> bool {
...
mod tests {
    ...
    fn coding_segmentor_stops_at_the_next_claim() {
        ...
        let opts = qingjian_core::Options::new();
        let ctx = qingjian_core::Context::default();
        let q = Query {
            input: "uUni",
            caret: 4,
            options: &opts,
        ...
        let mut seg = qingjian_core::Segmentation::default();
        ...
        let mut seg2 = qingjian_core::Segmentation::default();
        seg2.segments
            .push(qingjian_core::Segment::new(qingjian_core::Span::new(4, 6)));
        let q2 = Query {
            input: "ni hao uUni",
            caret: 11,
            options: &opts,
