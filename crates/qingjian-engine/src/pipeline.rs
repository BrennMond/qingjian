...
//! 架构位置：`qingjian-core::Pipeline` 的实现；`qingjian-core` 与 `qingjian-engine`
//! 之间的唯一接缝。
//!
//! # 顺序是数据，但不是任意的
//!
...
pub struct PipelineImpl {
    ...
    spelling: Option<Arc<dyn qingjian_core::Spelling>>,
...
impl PipelineImpl {
    ...
    fn segment_for_display(&self, input: &str) -> (String, Vec<Span>) {
        ...
        let mut buf = Vec::new();
        {
            let mut sink = qingjian_core::ExpansionSink::new(&mut buf, 1);
    ...
    /// 这里仍然调用**唯一的排序入口** [`qingjian_core::sort_candidates`]，
    ...
    fn append_predictions(&self, state: &mut SessionState, out: &mut Vec<Candidate>) {
        ...
        qingjian_core::sort_candidates(&mut block);
...
impl Pipeline for PipelineImpl {
    ...
    fn process_key(&mut self, state: &mut SessionState, key: &qingjian_core::Key) -> ProcessResult {
    ...
    fn compose(&mut self, state: &mut SessionState, out: &mut Vec<Candidate>) {
        ...
        // [`qingjian_core::Segmentor::body_start`]）。
        //
        ...
        // 见 `qingjian_core::Query` 的"为什么这里没有 composition"。
        let caret = state.composition.caret;
        ...
        //    给出的顺序（`Pipeline::compose` 的契约）。
        {
            ...
            if !self.rankers.is_empty() {
                ...
                #[cfg(debug_assertions)]
                debug_assert!(
                    !qingjian_core::has_cross_class_inversion(&before, out, Lane::Input),
                    "重排器把猜测候选顶到了精确匹配之前"
                );
            }

            // **排序**：唯一的排序入口（全序、稳定 ⇒ 可复现）。
            qingjian_core::sort_candidates(out);
...
mod tests {
    ...
    fn echo_fallback_is_present_but_last_for_unknown_input() {
        ...
        let literal = cands
            ...
            .find(|c| c.text == "zzz")
            .expect("原样上屏候选必须存在");
        assert_eq!(literal.origin, qingjian_core::Origin::Literal);
    }
    ...
    fn rejected_key_stops_the_chain() {
        let mut p = build();
        let mut state = SessionState::default();
        let ctrl_c = Key::press(
            qingjian_core::KeyCode::Char('c'),
            qingjian_core::Modifiers::CTRL,
        );
        assert_eq!(
    ...
    /// 而"预测从哪来"是 `qingjian-memory` 的事（那边有它自己的端到端测试）。
    struct FixedPredictor(Vec<(&'static str, i32)>);

    impl qingjian_core::MemoryStore for FixedPredictor {
        fn record(&self, _commit: &qingjian_core::Commit) {}
        fn lookup(&self, _key: &str) -> Vec<qingjian_core::MemoryEntry> {
            Vec::new()
        }
        fn forget(&self, _key: &str, _text: &str) {}
        fn predict_next(
            &self,
            _context: &qingjian_core::Context,
        ) -> Vec<qingjian_core::Prediction> {
            self.0
                .iter()
                .map(|(text, ml)| qingjian_core::Prediction {
                    text: (*text).to_owned(),
                    score: qingjian_core::Score::from_milli_log(*ml),
                    origin: qingjian_core::PredictionOrigin::Personal,
                })
                .collect()
        }
    }
