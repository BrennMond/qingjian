...
use qingjian_core::{Candidate, Filter, Query, Span, Tag};
    ...
    fn apply(&self, q: &Query<'_>, _span: Span, cands: &mut Vec<Candidate>) {
        ...
        for c in cands.iter() {
            for converted in self.convert_all(&c.text) {
                ...
                n.score = c
                    .score
                    .saturating_add(qingjian_core::Score::from_weight(self.weight));
                extra.push(n);
            }
        }
        cands.extend(extra);
...
mod tests {
    use super::*;
    use qingjian_core::{Context, Lane, Options, Origin, Score, SpellingAttr};
    ...
    fn merge_intersects_spelling_attrs() {
        ...
        assert!(
            !qingjian_core::is_exact(v[0].origin, v[0].attr) || v[0].attr == SpellingAttr::NORMAL,
            "交集规则"
        );
