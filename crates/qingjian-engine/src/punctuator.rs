...
//! 架构位置：`qingjian-core` 的 `Processor` / `Translator` 的实现。
//!
//! # 标点为什么不是"一个翻译器"就够
//!
...
impl qingjian_core::Processor for Punctuator {
...
impl PunctTranslator {
    ...
    fn push(text: &str, span: Span, out: &mut CandidateSink<'_>, score_milli: i32) {
        out.push(Candidate {
            ...
            span,
            lane: Lane::Input,
            kind: qingjian_core::CandidateKind::Punct,
            key: None,
        });
    }
}
...
impl Translator for PunctTranslator {
    ...
    fn accepts(&self, tags: &[qingjian_core::Tag]) -> bool {
        tags.contains(&"punct")
    }

    fn targets(&self) -> &[qingjian_core::Tag] {
        // `punct` 是引擎内置的标签名（RIME 也用同一个词）。
        const PUNCT: &[qingjian_core::Tag] = &["punct"];
        PUNCT
    }
}
...
mod tests {
    use super::*;
    use qingjian_core::{Key, Options, Processor};
    ...
    fn state_with(option: Option<(&str, bool)>) -> SessionState {
        ...
        if let Some((n, on)) = option {
            s.options.declare(qingjian_core::Switch::new(n, on));
        }
        s
    }
