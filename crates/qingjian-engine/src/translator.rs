...
//! 架构位置：`qingjian-core::Translator` 的实现。
//!
//! # 为什么是两族（而不是一族）
//!
...
impl Translator for ExactCodeTranslator {
    fn translate(&self, q: &Query<'_>, span: Span, out: &mut CandidateSink<'_>) {
        ...
        let key = qingjian_core::code_key(&self.alphabet, &code);
        for mut c in raw {
            ...
            // 实测就是这么发现的：`qingjian --check` 的冒烟测试从「十」变成了「木」。
            //
            // 数值与拼写层共用同一个常量：它就是"补全"这一类边的标准代价
            // （`ln 0.05 ≈ -3.0`）。
            if c.attr.contains(qingjian_core::SpellingAttr::COMPLETION) {
...
impl Translator for SpellingGraphTranslator {
    fn translate(&self, q: &Query<'_>, span: Span, out: &mut CandidateSink<'_>) {
        ...
        let mut expansions = Vec::new();
        {
            ...
            let limits = qingjian_core::PathLimits::default().with_completion(self.completion);
            let mut sink = qingjian_core::PathSink::new(&mut expansions, 512);
        ...
        for exp in &expansions {
            ...
            if completion
                ...
                && exp.attr == qingjian_core::SpellingAttr::NORMAL
            {
                let mut sink = CandidateSink::new(&mut raw, TRANSLATE_CAP);
            ...
            let key = qingjian_core::code_key(self.spelling.alphabet(), &exp.code);
            ...
            for mut c in raw {
                ...
                if completion && c.attr.contains(qingjian_core::SpellingAttr::COMPLETION) {
...
impl SpellingGraphTranslator {
    ...
    fn make_sentence(&self, input: &str, span: Span, out: &mut CandidateSink<'_>) {
        ...
        let mut probe: Vec<qingjian_core::SpellingPath> = Vec::new();
        {
            let mut sink = qingjian_core::PathSink::new(&mut probe, 64);
            self.spelling.expand_paths(
                input,
                qingjian_core::PathLimits::sentence_scan(),
                &mut sink,
            );
        }
        for p in &probe {
        ...
        for &start in &starts {
            let mut paths: Vec<qingjian_core::SpellingPath> = Vec::new();
            {
                let limit = qingjian_core::PathLimits::sentence_scan();
                let mut sink = qingjian_core::PathSink::new(&mut paths, limit.max_paths);
                self.spelling
...
impl Translator for TaggedTranslator {
    ...
    /// 现在流水线按 [`qingjian_core::Segmentor::body_start`] 填好正文，
...
mod tests {
    ...
    use qingjian_core::{Context, Options};
    ...
    fn echo_translator_always_yields_something() {
        ...
        assert!(
            qingjian_core::is_exact(buf[0].origin, buf[0].attr),
            "原样上屏不是猜测"
        );
