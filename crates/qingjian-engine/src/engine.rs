...
//! 架构位置：`qingjian-core::Engine` / `Session` / `SchemaCatalog` 的实现。
//!
//! # 会话如何兑现"上屏"
//!
...
impl SessionImpl {
    fn new(scheme: Arc<LoadedScheme>, inner: Arc<EngineInner>) -> Self {
        ...
        Self {
            ...
            scheme,
            pipeline,
            state: SessionState::new(qingjian_core::Composition::default(), options, context),
            candidates: Vec::new(),
            events: Vec::new(),
            last_commit: None,
        }
    ...
    fn realize_commit(
        ...
        if let PendingCommit::CommitComment { .. } = pending {
            ...
            let commit = Commit {
                ...
                input: self.state.composition.input.clone(),
                context: self.state.context.recent().to_vec(),
                origin: qingjian_core::Origin::Literal,
    ...
    fn finish_commit(&mut self, commit: Commit, consumed: usize) -> Commit {
        ...
        let is_punctuation_like = commit.origin == qingjian_core::Origin::Literal
...
impl Session for SessionImpl {
    fn process_key(&mut self, key: qingjian_core::Key) -> Outcome {
        self.state.pending_commit = None;
    ...
    fn composition(&self) -> &qingjian_core::Composition {
