...
//! 架构位置：`qingjian-core::Translator` / `Filter` 的实现。
//!
//! # 这一族零件从哪来
//!
...
// ─────────────────────────────────────────────────────────────────────────────
//
// 我们**不引第三方日期库**：`qingjian-core` / `qingjian-engine` 零依赖是
...
/// - 测试注入 [`qingjian_core::FrozenClock`]，于是"2026 年 11 月 29 日是星期几"
///   这类断言才能写。
///
...
pub struct UuidTranslator {
    /// 随机源（注入）。
    random: std::sync::Mutex<Box<dyn qingjian_core::RandomSource>>,
    /// 触发词。
    trigger: String,
    /// 本翻译器负责的标签。
    tags: Vec<Tag>,
...
impl UuidTranslator {
    ...
    #[must_use]
    pub fn new(
        random: Box<dyn qingjian_core::RandomSource>,
...
impl ReduceEnglishFilter {
    ...
    /// 它**——那是 GPL 项目的内容，而 Qingjian 是宽松许可。
    ///
    /// 这里是我们自己收的一小批，判据只有一条：
...
mod tests {
    ...
    use qingjian_core::{Context, FrozenClock, Lane, Options, SpellingAttr};
    ...
    fn uuid_translator() -> UuidTranslator {
        let mut tags = crate::tag::TagTable::new();
        UuidTranslator::new(
            Box::new(qingjian_core::DeterministicRandom::new(42)),
    ...
    fn uuid_is_deterministic_for_a_fixed_seed() {
        ...
        let other = UuidTranslator::new(
            Box::new(qingjian_core::DeterministicRandom::new(7)),
            &crate::spec::UuidSpec::default(),
            vec![tags.intern("uuid")],
        );
