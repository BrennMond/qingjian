...
/// # 取值是**实测反推**的，不是拍脑袋
///
/// 在真实默认拼音方案（**当时** 399 个音节 + `abbrev(take=1)`；
/// 生成器修好单字读音后是 405 个，见 `tools/README.md`「多音字」）
/// 的拼写表上，用 arena + 父指针的实现测（同一台机器，debug 构建）：
///
...
impl Default for ExpansionLimits {
    fn default() -> Self {
        Self {
            ...
            // 实测（默认方案 399 音节那版 + 一条缩写规则；405 音节下
            // `spelling_resource_bounds.rs` 仍全绿）：`nihao` 要出
