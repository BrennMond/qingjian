...
//! 架构位置：`qingjian-schemes`（装载器）与 `qingjian-engine`（零件实现）之间的契约。
//!
//! # 为什么数据定义与实现分开放
//!
//! RIME 的零件在方案里是这样配的：
...
impl TranslatorSpec {
    ...
    /// 驱动，见 `qingjian_core::PathLimits::completion`。
    ///
    /// 若将来决定与上游不同（例如"候选太多，默认关掉"），必须**明确写成
    /// Qingjian 的有意选择**并更新用户文档——不能再拿"上游也是关的"当理由。
    #[must_use]
...
mod tests {
    use super::*;
    use qingjian_core::{Key, KeyCode, Modifiers, NamedKey};
