...
use qingjian_engine::calc::CalcTranslator;
use qingjian_engine::spec::{At, CalcSpec};
...
fn the_result_vector_has_two_candidates_in_the_documented_order() {
    use qingjian_core::{CandidateSink, Context, Options, Query, Span, Translator};
    let t = translator();
    ...
    assert_eq!(buf[0].kind, qingjian_core::CandidateKind::Inline);
}
