...
//!
//! 中文职责：`qingjian_core::Lexicon` 的第二个实现——**索引常驻、词条按需读**。
//! English role: the second `Lexicon` implementation — resident index,
//! on-demand entry reads.
//! 架构位置：与 `qingjian_engine::InMemoryLexicon` **平级**。
//!
//! # 这个文件是"抽象是否成立"的验收
//!
                ...
                if let Ok(entries) = self.read_entries(idx, limit) {
                    for (text, score) in entries {
                        out.push(Candidate {
                            ...
                            span,
                            lane: Lane::Input,
                            kind: qingjian_core::CandidateKind::Completion,
                            key: None,
                        });
                    }
                }
...
mod tests {
    ...
    fn build(name: &str, entries: &[(&str, &[u16], f64)]) -> std::path::PathBuf {
        let p = std::env::temp_dir().join(format!("qingjian-lex-{name}.table"));
    ...
    fn truncation_keeps_the_best_ones() {
        let p = std::env::temp_dir().join("qingjian-lex-trunc.table");
        let _ = std::fs::remove_file(&p);
        compile(
            0,
            fp(),
    ...
    fn resident_index_is_small_compared_to_the_file() {
        let p = std::env::temp_dir().join("qingjian-lex-size.table");
        let _ = std::fs::remove_file(&p);
    ...
    fn a_non_table_file_is_refused_with_an_explanation() {
        let p = std::env::temp_dir().join("qingjian-lex-junk.table");
    ...
    fn empty_table_looks_up_nothing() {
        let p = std::env::temp_dir().join("qingjian-lex-empty.table");
        let _ = std::fs::remove_file(&p);
    ...
    fn write_bytes(name: &str, bytes: &[u8]) -> std::path::PathBuf {
        let p = std::env::temp_dir().join(format!("qingjian-lex-mal-{name}.table"));
        let _ = std::fs::remove_file(&p);
