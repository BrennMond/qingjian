...
- **反馈 ①：测试的承诺强于它真正检查的东西。**
  `spelling_resource_bounds.rs` 里那条孪生体测试原先对两边都做
  `sort()` + `dedup()`，验证的只是**音节集合**相等（顺序、重复项都看不见），
  而注释写的是"逐项一致"；`rules` 只断言了"非空"。
  现已改为：**有序**逐项比较、显式断言真实方案字母表**无重复项**、
  比较 **`rules` 正文**、再拿**解析出来的**演示体字母表与磁盘上的有序列表
  对照。四条断言都用破坏测试验证过会红（换顺序 / 加重复项 / 改规则）。
- **反馈 ②：审计表的部分行号仍然过时。**
  `docs/config-field-audit.md` 的 `scheme.rs` 引用在第一次核对**之后**又整体
  +8——原因是同一轮里给 `qingjian-engine/src/scheme.rs` 的 `entry()` 换了
  一段会编译的文档示例（就是上面那条），该文件里这一节引用的每一行都下移。
  行号已更正，并新增守卫
  `config_field_audit.rs::the_line_references_in_the_audit_table_still_point_at_their_fields`：
  先**按行**核对表里每一行的坐标（不能靠别的行替它出现——只查"文档里出现过
  这个坐标"的弱版本被破坏测试证明抓不住），再核对那个坐标在代码里必须包含的
  片段。两个方向都验证过会红。
