...
**重开已确认段**（审计明确要求）现已实现：`Session::reopen()` 把最近一次
上屏的**原始拼写**放回预编辑串并重新分析；取消（`reset` 或拼写中按 Esc）
会作废重开目标。**已知边界**（写在该方法的文档里，并有 `#[ignore]` 测试记录）：
...
这三条是**结构**上的缺口（需要提交历史与"确认段"两个类型），
不是实现能顺手补掉的。

**仍未做**：URL / 数字**识别器**与候选选择的交叉矩阵（识别器本身在
`regex_and_recognizer.rs` 里有覆盖）；候选覆盖**任意 span**（现在只支持
"从头消费到 consumed"）。
