...
| crate | **10 个** |
| Rust | 约 33500 行 |
| 词条 | **414525 条**（414111 词 + 414 补录单字），405 个编码单元 |
...
| **默认词库** | `schemes/qingjian-default/cn_dicts/generated.dict.yaml`：jieba 通用词表 + 8 份 THUOCL 分领域词表；简繁过滤（用 OpenCC 的 `TSCharacters` 滤掉繁体条目）。主词典 `pinyin.dict.yaml` 仍是**导入清单** | 音节表 **405 个**编码单元，由生成器反推并同步写回方案 |
...
**多音字：只取首选读音，不做覆盖**。`pinyin.txt` 给每个字一个有序读音表
（首要读音在前），生成器取**首个**，并在落盘前自检「码 = 首选读音」。
而**词级拼音不在任何一份可分发数据里**，所以**它不会把「银行」读成
`yín háng`**（那是另一份数据的事）——那类词由人工覆盖表兜，覆盖范围只到
枚举过的词条。曾经的"同声母、按音节频率微调"启发式已删除（它把「家」
编成了 `jie`，实测影响 838 个多音字），成因与回归见
`tools/README.md`「多音字」。这一条写在词库头部、`tools/README.md` 与
生成器的文档里，不藏在代码里。
...
| 25 | **我给对照数据放进了内核 crate**（`crates/qingjian-engine/tests/oracle/`） | `verify-no-scheme-data.sh` 当场拦下：**内核不许有数据文件**（D24）。"只是测试用"不是理由——门禁第六次抓到我。**⚠️ 2026-09-14 更正**：该门禁当时用的是**扩展名黑名单**，里面**没有 `.lua`**——于是同一目录下的 `trace.lua` / `trace2.lua`（上游 GPL-3.0 副本）漏了过去，一直留在公开仓库里。黑名单的失效方式是"漏"，白名单的失效方式是"吵"；已改白名单并双向反向验证（见 notices §5.1） |
