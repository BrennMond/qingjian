...
| --- | --- | --- | --- | --- | --- |
| `dictionary`（主实例） | `components.rs:705` | `file.rs:511`（内联 / 部署）→ `SchemeDef.dictionary` | `scheme.rs:285` 造词库；`translator.rs` 查询 | 全部集成测试 | — |
| `dictionary`（`@别名` 实例） | `components.rs:705` | **未装配** | **无** | — | ✅ **降级诊断**（`check_translator_specs`） |
| `enable_completion` / `enable_word_completion` | `components.rs:709-710` | `scheme.rs` 两条装配路径都读 | `translator.rs` 的 `completion` 分支 | `config_field_audit.rs::completion_reaches_both_translator_families` | — |
| `enable_sentence` | `components.rs:711` | 存进 spec | **拼音族不看**（上游同款）；**码表族未实现** | `config_field_audit.rs::a_missing_feature_is_reported_not_silently_ignored` | ✅ **降级诊断** |
| `initial_quality` | `components.rs:712` | `scheme.rs` 两条路径都 `.with_initial_quality(...)` | `translator.rs` 加对数域常数 | `initial_quality_actually_changes_the_scores`、`..._is_per_translator_instance_not_global` | — |
...
| `punctuator` | `components.rs:583` | `Punctuator` | `punctuator.rs` | `p3_pipeline.rs` | 缺段时给默认 |
...
| `reverse_lookup_filter@别名` | `components.rs:578` | `ReverseLookupFilter` + `ReverseLexicon` | `filter.rs` | `p3_pipeline.rs` | 数据源固定为主词库的反查索引 |
