...
>
> **文字形态：当前以简体中文为维护范围。** 开发者的日常书写与测试经验均限于
> 现行规范简体，因此随项目提供的数据、测试和体验验收只覆盖简体；这不是对繁体
> 使用者需求的否定。相关接口与配置项会保留，方便后续扩展；但在有熟悉该形态的
> 维护者和可验证的测试前，本项目不承诺繁体数据或体验的适配（见 `PLAN.md` D32）。

**当前状态：原型内核（研究阶段）。** 已有分层引擎、两族翻译器、方案装载、
紧凑词库、可选本地记忆，以及一批可重跑的对照工装与回归测试。

**开发说明。** 本项目由仍在学习 Rust 的个人发起，当前实现和文档大量借助 AI
编程助手完成；维护者尚未充分理解或独立验证每一处实现。因此，README 中的主张
应以可重跑的测试、基准和源码为准，而不应被视为生产可用性的保证。欢迎指出可复现
的问题、设计缺陷或不准确的表述。
...
4. **简体为当前维护范围，繁体保留扩展接口**：转换能力（`simplifier` 一类的
   组件与开关）保留并可配置；但由于缺少相应的使用经验和测试，本项目暂不维护
   繁体数据，也不承诺繁体体验的适配。
